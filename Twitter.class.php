...
class Twitter extends SocialMedia
{
    ...
    protected $cache_key = "twitter";

    function __construct($consumer_key, $consumer_secret, $user_token, $user_secret, $screen_name, $curl_ssl_verifypeer = false)
    {
        $this->consumer_key        = $consumer_key;
        $this->consumer_secret     = $consumer_secret;
        $this->curl_ssl_verifypeer = $curl_ssl_verifypeer;
    ...
    /**
     * @param boolean $curl_ssl_verifypeer
     */
    public function setCurlSslVerifypeer($curl_ssl_verifypeer)
    {
        $this->curl_ssl_verifypeer = $curl_ssl_verifypeer;
    }
    ...
    {
        return $this->curl_ssl_verifypeer;
    }
    ...
    public function loadPosts($start = 0, $count = 1)
    {
        $tmhOAuth = new tmhOAuth(array(
            ...
            'curl_ssl_verifypeer' => $this->curl_ssl_verifypeer
        ));
        ...
        if ($response) {
            ...
            for ($i = 0;$i < count($response);$i ++) {
                ...
                if ($i >= $start) {
                    ...
                    $is_retweet = isset($response[$i]["retweeted_status"]) && $response[$i]["retweeted_status"];
                    if ($is_retweet) {
                        $text = "RT @" . $response[$i]["retweeted_status"]["user"]["screen_name"] . ": " . $response[$i]["retweeted_status"]["text"];
                    }

                    $text           = $this->addUrls($text);
                    $latest_posts[] = array(
                        "date"    => strtotime($response[$i]["created_at"]),
                        "text"    => $text,
                        "url"     => "https://twitter.com/" . $this->screen_name,
                        "picture" => ((isset($response[$i]["media"]) && $response[$i]["media"]["type"] == "photo") ? $response[$i]["media"]["media_url_https"] : false)
                    );
                }
            }
        }
    ...
    /*
     * Adds links to usernames and hashtags
     */
